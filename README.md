...
| 工具 | 方式 |
|---|---|
| Cursor | 将 `prompts/universal.md` 内容粘贴到 Rules for AI |
| Codex CLI | 将 `prompts/universal.md` 内容设为系统指令 |
| ChatGPT / Claude 网页版 | 直接粘贴 `prompts/universal.md` 内容 |
...
pitchcraft/
├── .claude/
│   └── skills/pitchcraft/SKILL.md   # Claude Code 原生 skill
├── .claude-plugin/
...
│   └── pitchcraft.md                  # 安装命令
├── prompts/
│   └── universal.md                   # 通用 prompt（任意 AI 工具可用）
├── README.md
├── LICENSE
└── .gitignore
```
...
当前版本：v1.0 — 核心框架 + 5 种场景模板 + 工作流 + 检查表 + 跨平台安装
